/*----------------------------------------------------------------------------
 ...
 File:     ho_rev.c
 Revision: $Id: ho_rev.c 134 2009-03-03 14:25:24Z imosqueira $
...

{
    ...
    if (k compsize kax || rev_location_cnt compsize rax) {
        ...
        if ((T == NULL)||(Tqo == NULL)||(Ttemp == NULL)) {
            fprintf(DIAG_OUT,"ADOL-C error: cannot allocate %i + %i + %i bytes!\n",
                    (int)(k*sizeof(revreal)),
                    (int)(rev_location_cnt*sizeof(revreal*)),
                    (int)(rev_location_cnt*k*sizeof(revreal)));
            exit (-1);
        }
                    ...
                    if (coval <= 0.0) {
                        ...
                        if (coval - floor(coval) != 0) {
                            ...
                            if (coval == 1) {
                                ...
                                copyAndZeroset(k,Ares,Atemp);
                                inconv(k,Atemp,Targ,Aarg);
                                inconv(k,Atemp,Targ,Aarg);
                                if (coval == 3) {
                                    FOR_0_LE_i_LT_k
                                    printf(" i %d Aarg %f Targ %f Atemp %f \n",i,Aarg[i],Targ[i],Atemp[i]);
                                    conv(k,Aarg,Targ,Atemp);
                                    FOR_0_LE_i_LT_k
                                    printf(" Aarg %f Targ %f Atemp %f \n",Aarg[i],Targ[i],Atemp[i]);
                                    /*                      if(arg != res) */
                                    /*                       { inconv(k,Ares,Tres,Aarg); */
                                    /*                        FOR_0_LE_i_LT_k */
                                    /*                          ARES_INC = AQO_INC; */
    ...
    case eq_plus_av:                                        /* eq_plus_av */
        ...
        case eq_mult_av_a:                                    /* eq_mult_av_a */
            ...
            if ((arg >= res) && (arg < res+size)) { /* FIRST compute the case: res==arg */
        ...
        case mult_a_av:                                          /* mult_a_av */
            ...
            /* olvo 980929 new strategy to check for overwrites
               (changes computation order) */
            if ((arg2 >= res) && (arg2 < res+size)) { /* FIRST compute the case: res==arg2 */
