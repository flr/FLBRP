/*----------------------------------------------------------------------------
 ...
 Revision: $Id: tayutil.c 134 2009-03-03 14:25:24Z imosqueira $
...
void write_taylor( locint loc, int depth ) {
    ...
    for (i=0;i<depth;i++) { /* olvo 980708 changed order */
...
void write_scaylor( revreal x ) { /* olvo 980708 changed order */
    if ((++cur_vs_data->taylor_cnt) == cur_vs_data->T_buf_size)
        T_put_block(cur_vs_data->T_buf_size);
...
void write_scaylors(double *x, int size) {
    int i;
    for(i=0; i<size; i++) { /* olvo 980708 changed order */
        if ((++cur_vs_data->taylor_cnt) == cur_vs_data->T_buf_size)
            T_put_block(cur_vs_data->T_buf_size);
