/*----------------------------------------------------------------------------
 ...
 File:     taputil.c
 Revision: $Id: taputil.c 134 2009-03-03 14:25:24Z imosqueira $
...
/*--------------------------------------------------------------------------*/
static void set_up_buffers( short tag, int buffer_size ) { /* Return old memory ... if used */
    if (op_tape[tag])
...
void tapestats( short tag,int *tape_stat ) {
    ...
    if (stats[tag] == 0) { /* Tape number does not exist , so read in tape data */
...
/*--------------------------------------------------------------------------*/
/* Locations */
void put_locint( locint loc ) { /*if (loc_ptr == buff_size) put_locint_block(buff_size); */
...
/*--------------------------------------------------------------------------*/
/* Values */
void put_val( double r_val ) { /* if (real_ptr == buff_size) put_val_block(buff_size); */
