...
#    fn=list("SSB"    =function(x) ssb(x)/refpts(x)[scale,"ssb"],
# 				   "yield"  =function(x) yield(x)/refpts(x)[scale,  "yield"],
# 				   "rec"    =function(x) rec(  x)/refpts(x)[scale,    "rec"],
# 				   "harvest"=function(x) fbar( x)/refpts(x)[scale,"harvest"]),...) {
#   
