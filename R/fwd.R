...
setMethod("fwd", signature(object="FLBRP", ctrl="missing"),
    function(object,
               ...
               availability=NULL,maxF=2.0,fbar="missing",f=fbar)
    {                          

    if (!missing(fbar)) fbar(object)=fbar  
    ctrl=fbar(object)[,-1]  
    stk =as(object,"FLStock") 
    res =FLash:::fwd(stk,f=ctrl,sr=sr,sr.residuals=sr.residuals,sr.residuals.mult=sr.residuals.mult,availability=availability,maxF=maxF)  
